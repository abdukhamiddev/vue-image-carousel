...
const { pause, resume } = useRafFn(() => {
    ...
    <div>
        <Cross />
        <div class="flex items-center h-screen" :class="{ 'pointer-events-none': dragging }">
            <div class="flex items-center justify-start w-full space-x-7 pl-[50vw] select-none"
                :style="{ transform: `translateX(calc(${positionX}px + -9vw))` }">
                <Item :key="item.image" v-for="(item, index) in items" :ref="(comp) => (sliderItems[index] = comp)"
                    :item="item" :index="index" :x="positionX" />
            </div>
        </div>
